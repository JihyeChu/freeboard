...
//	DAO 클래스의 insert sql 명령을 실행하는 메소드를 호출하는 메소드
...
//	메인글 목록을 얻어오는 DAO 클래스의 select sql 명령을 실행하는 메소드를 호출하는 메소드
...
//	증가시키는 DAO 클래스의 update sql 명령을 실행하는 메소드를 호출하는 메소드
...
//	selectByIdx.jsp에서 호출되는 메인글의 글번호를 넘겨받고 mapper를 얻어온 후 메인글 1건을 얻어오는
//	DAO 클래스의 select sql 명령을 실행하는 메소드를 호출하는 메소드
...
//	DAO 클래스의 delete sql 명령을 실행하는 메소드를 호출하는 메소드
...
//	list.jsp에서 호출되는 mapper를 얻어온 후 모든 공지글을 얻어오는 DAO 클래스의 select sql 명령을
//	실행하는 메소드를 호출하는 메소드
	public ArrayList<FreeboardVO> selectNotice() {
		System.out.println("FreeboardService 클래스의 selectNotice() 메소드");
		SqlSession mapper = MySession.getSession();
		ArrayList<FreeboardVO> notice = FreeboardDAO.selectNotice(mapper);
		mapper.close();
		return notice;
	}
	
//	updateOK.jsp에서 호출되는 메인글을 수정할 수정할 정보가 저장된 객체를 넘겨받고 mapper를 
//	얻어온 후 메인글 1건을 수정하는 DAO 클래스의 update sql 명령을 실행하는 메소드를 호출하는
//	메소드
